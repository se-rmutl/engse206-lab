...
เอกสารนี้อธิบายโครงสร้าง Backend Server ของ Agent Wallboard System ที่ใช้ **Node.js + Express + Socket.io** พร้อมตัวอย่างโค้ดที่สมบูรณ์และพร้อมใช้งาน เชื่อมต่อกับ SQLite และ MongoDB ตาม schema ที่กำหนดใน 6.6.4

**เวอร์ชัน:** 2.0 (Updated with retry logic, error handling, and rate limiting)

### 🎓 เป้าหมายการเรียนรู้
- ✅ เข้าใจโครงสร้าง Backend Server ที่ทำงานได้จริง
- ✅ รู้วิธีเชื่อมต่อและใช้งาน databases ทั้งสองตัว
- ✅ เข้าใจ API endpoints และวิธีเรียกใช้
- ✅ เข้าใจ WebSocket สำหรับ real-time communication
- ✅ สามารถทดสอบและ debug Backend ได้
...
│
├── ⚙️ config/
│   └── database.js                 # Database connections (with retry)
│
...
```
Client Request → Rate Limiter → CORS → Auth Middleware → Route Handler 
→ Database Model → Database (SQLite/MongoDB) → JSON Response
```
      ...
      try {
        ...
        if (!fromCode || !content || !type) {
          socket.emit('message_error', { 
            message: 'fromCode, content, and type are required' 
          });
          return;
        }
        ...
        if (type === 'direct' && toCode) {
          ...
          if (targetSocketId) {
            console.log(`📤 Sending direct message to ${toCode} (socket: ${targetSocketId})`);
            ...
            console.log(`✅ Message delivered to ${toCode}`);
          } else {
            console.log(`⚠️  Agent ${toCode} is offline - message saved but not delivered`);
            deliveryStatus = 'offline';
          }
          
        } else if (type === 'broadcast') {
          console.log(`📢 Broadcasting message to team ${toTeamId}`);
          
          // Get all online agents in the team
          let deliveredCount = 0;
          activeConnections.forEach((socketId, agentCode) => {
            // Broadcast to all except sender
            if (agentCode !== fromCode.toUpperCase()) {
          ...
          deliveryStatus = deliveredCount > 0 ? 'delivered' : 'no_recipients';
          console.log(`✅ Broadcast message delivered to ${deliveredCount} agents`);
        }
        ...
        console.log(`💬 Message from ${fromCode} (${type}) - Status: ${deliveryStatus}`);
...
หลังจาก Backend พร้อมแล้ว:

1. ✅ **Backend Server พร้อมใช้งาน**
2. ⏭️ **[6.6.3.1 Frontend - Electron Agent App](./6.6.3.1-Frontend-Applications-Electron-Agent-App.md)** - เชื่อมต่อ Agent Desktop App
3. ⏭️ **[6.6.3.2 Frontend - Supervisor Dashboard](./6.6.3.2-Frontend-Applications-Supervisor-Dashboard.md)** - เชื่อมต่อ Supervisor Web Dashboard
...
**🚀 Backend Server v2.0 พร้อมแล้ว! ต่อไป: Frontend Development**