...
function App() {
  ...
  useEffect(() => {
    ...
    const handlers = {
      ...
      // ✅ แก้ไข: เพิ่ม logging และ validation
      new_message: (message) => {
        ...
        if (!isLoggedInRef.current) {
          logger.warn('⚠️  Received message but user not logged in');
          return;
        }
        
        // ✅ แก้ไข: เพิ่ม message ใน state พร้อม logging
        setMessages(prev => {
          logger.log('📝 Adding message to state. Current count:', prev.length);
          
          // Check for duplicates
          const isDuplicate = prev.some(m => m._id === message._id || m.messageId === message.messageId);
          if (isDuplicate) {
            logger.warn('⚠️  Duplicate message detected, skipping:', message._id);
            return prev;
          }
          
          const newMessages = [message, ...prev];
          logger.log('✅ Message added. New count:', newMessages.length);
          return newMessages;
        });
        
        // ✅ แก้ไข: แสดง notification พร้อม error handling
        try {
          const notificationTitle = message.type === 'broadcast' 
            ? `Broadcast from ${message.fromCode}`
            : `Message from ${message.fromCode}`;
          
          logger.log('🔔 Attempting to show notification:', notificationTitle);
      ...
      message_error: (error) => {
        ...
        setError(`Message error: ${error.message || 'Unknown error'}`);
      },
...
// ✅ แก้ไข: Auto-scroll เมื่อมี message ใหม่
  ...
  // ✅ เพิ่ม: ตรวจสอบว่าผู้ใช้ scroll หรือไม่
  const handleScroll = () => {
  ...
  return (
    <div className="message-panel">
      ...
      {/* ✅ เพิ่ม: onScroll handler และ ref */}
      <div 
...
export const showDesktopNotification = async (title, body) => {
  try {
    console.log('🔔 showDesktopNotification called:', { title, body });
    ...
    if (window.electronAPI && window.electronAPI.showNotification) {
      ...
      if (result.success) {
        console.log('✅ Electron notification shown');
        return true;
      } else {
        console.error('❌ Electron notification failed:', result.error);
      }
    }
